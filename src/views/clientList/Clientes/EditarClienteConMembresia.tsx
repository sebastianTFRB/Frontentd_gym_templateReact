...
function resolveFotoSrc(src?: string | null): string | null {
  if (!src) return null;
  if (src.startsWith("data:")) return src; // dataURL
  if (/^https?:\/\//i.test(src)) return src; // absoluta
  if (src.startsWith("/")) return API_ORIGIN + src; // /media/...
...
async function uploadBase64ToBackend(
  ...
  return data.ruta as string; // ej: "/media/fotos/123.jpg"
}
...
export default function EditarClienteConMembresia() {
      ...
      .then(([cliRes, ventasRes, membRes]) => {
        ...
        if (last) {
          ...
          if (last.fecha_inicio) {
            ...
            const iso = `${di.getFullYear()}-${String(di.getMonth() + 1).padStart(2, "0")}-${String(
              di.getDate()
            ).padStart(2, "0")}`;
            setFechaInicio(iso);
          }
          if (last.fecha_fin) {
            ...
            const iso = `${df.getFullYear()}-${String(df.getMonth() + 1).padStart(2, "0")}-${String(
              df.getDate()
            ).padStart(2, "0")}`;
            setFechaFin(iso);
          }
  ...
  // Recalcular fecha fin al cambiar inicio (comodidad)
  useEffect(() => {
  ...
  const precioInvalid = useMemo(() => {
    if (precioFinal.trim() === "") return false; // opcional
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      // 1) Si se capturó/subió nueva foto, súbela y obtén ruta
      ...
      if (fotoBase64Nueva) {
        ...
        fotografiaRuta = ruta; // ej: "/media/fotos/CC.jpg"
        setFotoRutaActual(ruta);
        setInfo("Foto actualizada.");
      }

      // 2) Armar payload (incluimos 'fotografia' si hubo nueva)
      const payload: any = {
        cliente: {
          nombre: nombre.trim(),
          ...
          huella_base64: "",
          ...(fotografiaRuta ? { fotografia: fotografiaRuta } : {}),
        },
        venta: {
          id: ventaId,
      ...
      } as ActualizarClienteYVentaRequest;
        ...
        <form onSubmit={onSubmit} className="space-y-8">
          ...
          <section>
            ...
            <div className="flex items-center gap-6 mb-3">
              <div>
                <div className="text-xs mb-1 opacity-70">Actual</div>
                {fotoRutaActual ? (
                  <img
                      ...
                      (e.currentTarget as HTMLImageElement).src =
                        ...
                        encodeURIComponent(
                          `<svg xmlns='http://www.w3.org/2000/svg' width='128' height='128'>
                             <rect width='100%' height='100%' rx='16' fill='#2d333b'/>
                           </svg>`
                        );
                    }}
                  />
              ...
              {fotoPreviewUrl && (
                <div>
                  <div className="text-xs mb-1 opacity-70">Nueva</div>
                  <img
                    src={fotoPreviewUrl}
                    alt="nueva"
                    className="h-32 w-32 object-cover rounded-xl border"
            ...
            <p className="text-xs opacity-70 mt-2">
              La foto se guardará cuando presiones <strong>Guardar</strong>.
            </p>
          </section>
